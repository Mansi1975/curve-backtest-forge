  ...
  return <section id="backtest" className="relative py-24 bg-gray-900">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-gray-300 max-w-3xl mx-auto leading-relaxed">Test your strategies with historical data and get institutional-grade performance analytics.</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              {strategies.map(strategy => <Card key={strategy.id} className={`p-6 cursor-pointer transition-all duration-300 hover:border-emerald-500/50 ${selectedStrategy === strategy.id ? 'bg-emerald-900/30 border-emerald-500' : 'bg-gray-800/50 border-gray-700'}`} onClick={() => setSelectedStrategy(strategy.id)}>
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="text-lg font-semibold text-white">{strategy.name}</h4>
                    <div className={`w-4 h-4 rounded-full border-2 ${selectedStrategy === strategy.id ? 'bg-emerald-500 border-emerald-500' : 'border-gray-400'}`}></div>
                  </div>
